...
public static partial class Mailslots
{
    ...
    ///  Maximum size, in bytes, of messages that can be posted to the mailslot. 0 means any size.
    /// </param>
    /// <param name="readTimeout">
    ///  Timeout, in milliseconds, that a read operation will wait for a message to be posted. 0 means do not wait,
    ///  uint.MaxValue means wait indefinitely.
    /// </param>
    ...
    public static unsafe MailslotInfo GetMailslotInfo(SafeMailslotHandle mailslotHandle)
    {
        ...
        Error.ThrowLastErrorIfFalse(
            TerraFXWindows.GetMailslotInfo(
                hMailslot: mailslotHandle.ToHANDLE(),
    ...
    /// </summary>
    /// <param name="readTimeout">
    ///  Timeout for read operations in milliseconds. Set to uint.MaxValue for infinite timeout.
    /// </param>
    public static void SetMailslotTimeout(SafeMailslotHandle mailslotHandle, uint readTimeout)
        => Error.ThrowLastErrorIfFalse(TerraFXWindows.SetMailslotInfo(mailslotHandle.ToHANDLE(), readTimeout));
}
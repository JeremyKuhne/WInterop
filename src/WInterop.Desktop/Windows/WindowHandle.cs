...
public readonly struct WindowHandle : IHandle<WindowHandle>
{
    ...
    /// </summary>
    public static WindowHandle Top = new(default);

    /// <summary>
    ///  For placing windows at the bottom of the Z order. [HWND_BOTTOM]
    /// </summary>
    public static WindowHandle Bottom = new(new HWND(new IntPtr(1)));

    /// <summary>
    ///  For placing windows behind all topmost windows (if not already non-topmost). [HWND_NOTOPMOST]
    /// </summary>
    public static WindowHandle NoTopMost = new(new HWND(new IntPtr(-2)));

    /// <summary>
    ///  For placing windows above all non-topmost windows. [HWND_TOPMOST]
    /// </summary>
    public static WindowHandle TopMost = new(new HWND(new IntPtr(-1)));

    /// <summary>
    ///  Used to create and enumerate message only windows. [HWND_MESSAGE]
    /// </summary>
    public static WindowHandle Message = new(new HWND(new IntPtr(-3)));

    /// <summary>
    ///  Used for sending broadcast messages. [HWND_BROADCAST]
    /// </summary>
    public static WindowHandle Broadcast = new(new HWND(new IntPtr(0xffff)));

    public static WindowHandle Null = new(default);

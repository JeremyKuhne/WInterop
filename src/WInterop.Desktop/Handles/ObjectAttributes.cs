...
{
    /// <summary>
    ///  This handle can be inherited by child processes of the current process. [OBJ_INHERIT]
    /// </summary>
    Inherit = 0x00000002,

    /// <summary>
    ///  This flag only applies to objects that are named within the object manager. By default, such objects are
    ///  deleted when all open handles to them are closed. If this flag is specified, the object is not deleted
    ///  when all open handles are closed. [OBJ_PERMANENT]
    /// </summary>
    ...
    /// <summary>
    ///  Create on existing object should open, not fail with STATUS_OBJECT_NAME_COLLISION. [OBJ_OPENIF]
    /// </summary>
    OpenIf = 0x00000080,
